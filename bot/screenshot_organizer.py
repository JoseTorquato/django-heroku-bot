...
def fetch_events(start_date, end_date, username):
    ...
    # time.
    print(username)
    if os.path.exists(f'token_{username}.json'):
        creds = Credentials.from_authorized_user_file(f'token_{username}.json', SCOPES)
    ...
    if not creds or not creds.valid:
        ...
        with open(f'token_{username}.json', 'w') as token:
...
if __name__ == '__main__':
    ...
    try:
        ...
        start_date = now
        end_date = now.replace(hour=23, minute=59, second=59, microsecond=999999)
        events = fetch_events(start_date, end_date, 'josetorquato')
...
def google_calendar(username):
    now = datetime.datetime.utcnow()
    start_date = now
    end_date = now.replace(hour=23, minute=59, second=59, microsecond=999999)
    return fetch_events(start_date, end_date, username)


def google_calendar_bot(username):
    now = datetime.datetime.utcnow()
    start_date = now
    end_date = now.replace(hour=23, minute=59, second=59, microsecond=999999)
    return fetch_events(start_date, end_date, username)
